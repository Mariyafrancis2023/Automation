...
public class MyWishList extends TestBase{
	...
	public WebElement getCellElementFromTable(MyWishListTableHeader column, String uniqueColumnElementName) {
		...
		for(int i =0; i < rowsElement.size(); i++) {
			...
			//enter the index of column in get() from which we need to get the co-responding value. 
			if(cells.get(1).getText().equals(uniqueColumnElementName)) {
	...
	private int getIndexOfColumn(MyWishListTableHeader columnName) {
